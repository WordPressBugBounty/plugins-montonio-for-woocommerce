...
class WC_Montonio_Shipping_Route_Setup {
     ...
     *
     * Re-validates zone names at execute time to prevent duplicates (TOCTOU fix).
    ...
    public static function ajax_execute_html() {
        ...
        if ( empty( $plan ) || ! is_array( $plan ) || empty( $plan['zones'] ) ) {
            wp_send_json_error( array( 'message' => __( 'Invalid plan data.', 'montonio-for-woocommerce' ) ) );
        }
        ...
        if ( ! empty( $plan['has_dynamic_methods'] ) ) {
            if ( empty( $plan['default_dimensions'] ) || ! is_array( $plan['default_dimensions'] ) ) {
                wp_send_json_error( array(
                    'message' => __( 'Please fill in all package dimension fields with values greater than 0.', 'montonio-for-woocommerce' ),
                ) );
            }
        ...
        // Re-validate zone names before creating (TOCTOU fix)
        $existing_names = self::get_existing_zone_names();
        $skipped        = array();
        ...
        foreach ( $plan['zones'] as $index => $zone_data ) {
            ...
            if ( in_array( $zone_data['name'], $existing_names, true ) ) {
                $skipped[]                      = array( 'name' => $zone_data['name'] );
                $plan['zones'][ $index ]['skipped'] = true;
            } else {
                $existing_names[] = $zone_data['name'];
            }
        }

        try {
            $result = self::execute_plan( $plan );
            $html   = WC_Montonio_Shipping_Route_Setup_View::render_summary( $result, $skipped );
    ...
    public static function compute_plan() {
        ...
        $existing_zones = self::get_existing_zone_names();
        foreach ( $zones as &$zone ) {
            $zone['skipped'] = in_array( $zone['name'], $existing_zones, true );
        }
        unset( $zone );
    ...
    /**
     * Get existing WooCommerce shipping zone names.
     *
     * @since 9.4.0
     * @return array Array of existing zone names.
     */
    private static function get_existing_zone_names() {
        $names      = array();
